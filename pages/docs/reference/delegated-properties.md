...
有一些常见的属性，虽然我们可以在每次需要的时候手动实现它们，
但是如果我们只需实现一次即可，并顺带放入一个库，就更好了。例如包括：

* 延迟属性（lazy properties）: 其值只有在第一次访问时才会开始计算；
...
请注意，自 Kotlin 1.1 起你可以在函数或代码块中声明一个委托属性，因此它不一定非得是类的成员。
