...
**反馈循环**。 通过弃用周期需要付出很大的努力，因此我们希望最大限度地减少将来不兼容变更的数量。除了使用我们的最佳判断之外，我们相信在现实生活中试用是验证设计的最佳方法。在最终定论之前，我们希望已经实战测试过。这就是为什么我们利用每个机会在语言的生产版本中提供我们早期版设计，只是 in one of the  _pre-stable_ statuses: [Experimental, Alpha, or Beta](components-stability.html)。这些特性并不稳定，可以随时更改，选择使用它们的用户明确表示已准备好了应对未来的迁移问题。这些用户提供了宝贵的反馈，而我们收集这些反馈来迭代设计并使其坚如磐石。
...
增量版本带来了工具方面的更新（通常包括特性），性能改进和错误修复。我们试图使这些版本彼此兼容，因此对编译器的更改主要是优化和添加/删除警告。Pre-stable 特性可以随时被添加、删除或更改。

特性发布通常会添加新特性，并且可能会删除或更改以前不推荐使用的特性。某项特性从 pre-stable 到稳定版的过渡也包含在特性版本的发布中。
...
根据上述反馈环原则，我们在语言的开放和发行版本中对设计进行迭代，其中某些特性具有 one of the _pre-stable_ statuses 并且可以更改。这些特性可以随时被添加、更改或删除，不会发出警告。我们尽量确保 pre-stable 特性不会被用户意外使用。此类特性通常需要在代码或项目配置中进行某种类型的显式选择。

Pre-stable 特性通常会在经过几次迭代后逐渐达到稳定状态。
