...
首先，需要导入该 framework 到 `General` 选项的 *target*
配置。选择 `Linked Frameworks and Libraries` 选项来导入<!--
-->我的 framework。这将使 Xcode 查看我们的 framework 并同时解决来自
...
首先，需要导入编译好的 framework 到 Xcode 工程。为此<!--
-->将 framework 添加到 *target* 配置页的 `General` 选项的
`Embedded Binaries` 块。
