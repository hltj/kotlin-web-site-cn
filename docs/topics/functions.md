...
-->既可以作为具名参数在括号内传入，也可以在[括号外](lambdas.md#passing-a-lambda-to-the-last-parameter)传入：
...
>
> 完整的优先级层次结构请参见其[语法参考](grammar.md#expressions)。
{:.note}
...
关于泛型函数的更多信息参见[泛型](generics.md)。

## 内联函数

内联函数在[这里](inline-functions.md)讲述。

## 扩展函数

扩展函数在[其自有章节](extensions.md)讲述。

## 高阶函数和 Lambda 表达式

高阶函数和 Lambda 表达式在[其自有章节](lambdas.md)讲述。
