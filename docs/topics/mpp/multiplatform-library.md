...
-->将原始数据（字符串和字节数组）转换为 [Base64](https://en.wikipedia.org/wiki/Base64) 格式，该格式可用于
...
在 JS 上我们将使用原生的 [Buffer](https://nodejs.org/docs/latest/api/buffer.html) API 而在 Kotlin/Native 我们会<!--
-->编写自己的实现。
...
了解关于[平台特定的声明](../../reference/mpp-connect-to-apis.md)的更多信息。
...
在通用的 Native 平台上，我们没有可用的别人编写的实现，所以我们必须自己写一个。
...
至此，我们的库已经成功发布并且所有的 Kotlin 工程都可以依赖它，无论它是另外一个公共<!--
-->库还是 JVM、JS 或者是 Native 应用程序。
