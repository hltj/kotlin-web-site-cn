...
在 Kotlin 1.1 中引入的协程，一种全新的编写异步、非阻塞（以及更多）代码的方式。在这篇教程中会通过一些使用 Kotlin 协程的基础示例来帮助我们学习 `kotlinx.coroutines` 库，它是现有 Java 库的帮助程序和包装器的集合。
...
![Create a new project](new_gradle_project_jvm.png)

接下来跟随向导的步伐。你将根据[这篇文档](gradle.md)配置使用启用 Kotlin 的 `build.gradle` 文件。
...
这个库已经发布到了 Bintray JCenter 仓库，因此添加以下内容：
...
![Create a new project](new_mvn_project_jvm.png)

接下来跟随向导的步伐。你将根据[这篇文档](maven.md)配置使用启用 Kotlin 的 `pom.xml` 文件。
确保它配置了 Kotlin 1.3 或者更高版本。
