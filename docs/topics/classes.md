...
如果你不希望你的类<!--
-->有一个公有构造函数，那么声明一个带有非默认可见性的空的主构造函数：

```kotlin
class DontCreateMe private constructor () { /*……*/ }
```

> 在 JVM 上，如果主构造函数的所有的参数都有默认值，编译器会生成
...
创建嵌套类、内部类与匿名内部类的类实例在[嵌套类](nested-classes.md)中有述。
...
-->函数（例如，工厂方法），你可以把它写成该类内[对象声明](object-declarations.md)<!--
-->中的一员。
